...
def forward_checking_prop_singleton(state, verbose=False):
    ...
    queue = [ x for x in all_vars if x.domain_size() == 1 ]
    for var in all_vars:
    ...
    while len(queue)>0:
        X = queue[0]
        all_bcs = state.get_constraints_by_name(X.get_name())
        for bc in all_bcs:
            Y = state.get_variable_by_name(bc.get_variable_i_name() if bc.get_variable_i_name() != X.get_name() else bc.get_variable_j_name())
            for value in Y.get_domain():
                if not bc.check(state,value,X._domain[0]):
                    Y.reduce_domain(value)
                    domain_len = len(Y.get_domain())
                    if domain_len==0:
                        return False
                    elif domain_len==1:
                        queue.append(Y)
...
"""
for i in senate_people[:5]:
    print i,"\n"

for i in senate_votes[:5]:
    print i,"\n"
"""
...
def information_disorder(yes, no):
    ...
    all_types = set(yes+no)

    typ_in_yes = []
    typ_in_no = []
    
    for type_ in all_types:
        typ_in_yes.append(yes.count(type_))
        typ_in_no.append(no.count(type_))
        
    disorder_in_yes = 0
    disorder_in_no = 0

    for i in typ_in_yes:
        fraction = (i/float(Nb1))
        if fraction != 0:
            disorder_in_yes -= fraction*math.log(fraction,2)
        
    for i in typ_in_no:
        fraction = (i/float(Nb2))
        if fraction != 0:
            disorder_in_no -= fraction*math.log(fraction,2)

    return (Nb1/float(Nt))*disorder_in_yes + (Nb2/float(Nt))*disorder_in_no
