...
print senate_votes[0]
print len(senate_people[0]['votes']), "vs", len(senate_votes)
"""
...
def most_misclassified(classifier, n=5):
    ...
    """
    data_w = deepcopy(classifier.data_weights)
    data_w.sort(reverse=True)
    mmc = []
    for w in data_w[:5]:
        mmc.append(legislator_info(classifier.data[classifier.data_weights.index(w)]))
...
def describe_and_classify(filename, learners):
    ...
    results = Orange.evaluation.testing.cross_validation(classifiers, data,folds=min(10,len(data)))
...
learners = {
    ...
    "knn" : Orange.classification.knn.kNNLearner(k = 10),
...
learners2 = {
    "svml": orange.SVMLearner(kernel_type = orange.SVMLearner.Linear,
                              probability = True, svm_type=SVM_TYPE),
    "svmp3":orange.SVMLearner(kernel_type = orange.SVMLearner.Polynomial,degree = 3)}
