...
class Neuron(DifferentiableElement):
    ...
    def compute_output(self):
        ...
        X = 0
        for input_ , weight_ in zip(self.get_inputs(), self.get_weights()):
            X += input_.output() * weight_.get_value()
    ...
    def compute_doutdx(self, elem):
        ...
        if self.has_weight(elem):
            for i, weight_ in enumerate(self.get_weights()):
                if weight_.get_name() == elem.get_name():
                    return self.output() * (1 - self.output()) * self.get_inputs()[i].output()
            ...
            for i, w in enumerate(self.get_weights()):
                if self.isa_descendant_weight_of(elem, w):
                    result += self.get_inputs()[i].dOutdX(elem) *w.get_value()
                    
            return self.output() * (1 - self.output()) * result
...
class PerformanceElem(DifferentiableElement):
    ...
    def output(self):
        ...
        returns: number (float/int)
        """
        return -1*0.5*((self.my_desired_val - self.get_input().output())**2)
